...
type F = { reference: string; libelle: string; description: string; prixHT: string; tauxTVA: string; stock: string; stockMin: string; unite: string }
const EMPTY: F = { reference: '', libelle: '', description: '', prixHT: '', tauxTVA: '0', stock: '0', stockMin: '5', unite: 'piece' }
...
export default function ProduitsPage() {
  ...
  function openEdit(p: Produit) {
    setForm({ reference: p.reference, libelle: p.libelle, description: '', prixHT: String(p.prixHT), tauxTVA: '0', stock: String(p.stock), stockMin: String(p.stockMin), unite: p.unite })
    setEditId(p.id); setShowModal(true)
  }

  async function handleSave(e: React.FormEvent) {
    e.preventDefault(); setSaving(true)
    const payload = { reference: form.reference, libelle: form.libelle, prixHT: parseFloat(form.prixHT), tauxTVA: parseFloat(form.tauxTVA), stock: parseInt(form.stock), stockMin: parseInt(form.stockMin), unite: form.unite }
              ...
              {produits.map(p => (
                <tr key={p.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-3 max-w-[150px]">
                    <p className="font-medium text-gray-800 truncate">{p.libelle}</p>
                  </td>
